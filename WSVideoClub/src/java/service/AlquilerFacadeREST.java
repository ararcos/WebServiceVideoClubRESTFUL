...
public class AlquilerFacadeREST extends AbstractFacade<Alquiler> {
    ...
    @Path("{cedula}")
    @Produces({MediaType.APPLICATION_JSON})
    public List<Alquiler> buscar(@PathParam("cedula") String cedula) {
        
        Predicate<Alquiler> condicion = nuevo -> !
                nuevo.getSocId().getSocCedula().equals(cedula);
        List<Alquiler> peliculas = findAll();
        peliculas.removeIf(condicion);
        return peliculas;
    }
